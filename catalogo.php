<html>

    <body>
   

<div class="container">
  <div class="row">

<div class="card col-sm-3" style="width: 18rem;">
  <img class="card-img-top" src="./public/images/vidrios.png" alt="Card image cap">
  <div class="card-body">
    <h5 class="card-title">VIDRIOS</h5>
    <p class="card-text"></p>
    <a href="index.php?p=vidrios" class="btn btn-primary">Ver mas</a>
  </div>
</div>
<div class="col-sm-1"></div>

<div class="card col-sm-3" style="width: 18rem;">
  <img class="card-img-top" src="./public/images/divicion.jpg" alt="Card image cap" >
  <div class="card-body">
    <h5 class="card-title">DIVICIONES DE BAÑO</h5>
    <p class="card-text"></p>
    <a href="index.php?p=baño" class="btn btn-primary">Ver mas</a>
  </div>
</div>

<div class="col-sm-1"></div>

<div class="card col-sm-3" style="width: 18rem;">
  <img class="card-img-top" src="./public/images/pollos.jpg" alt="Card image cap" style="width: 220px; ">
  <div class="card-body">
    <h5 class="card-title">POLLOS DE COCINA</h5>
    <p class="card-text "></p>
    <a href="index.php?p=meson" class="btn btn-primary">Ver mas</a>
  </div>

  <div class="col-sm-1"></div>


  </div>
</div>
</div>
</div>


    </body>
</html>